...
struct LocalHelpIndex: Sendable {
    ...
    static let defaultSections: [HelpSection] = [
        HelpSection(id: "getting-started", title: "Getting Started", text: "Octonaut is a native Reddit reader for iPhone and iPad. Use Posts to browse Home, Popular, All, and community feeds. Search uses Reddit only after you submit a query. Feed content can be read offline after it has been cached."),
        HelpSection(id: "accounts-login", title: "Accounts and Login", text: "Octonaut signs into Reddit's website inside a temporary private web view. It never reads your password. The resulting session is stored in the device-only Keychain. Each saved account has its own session and account-scoped data."),
        ...
        HelpSection(id: "on-device-intelligence", title: "On-device Intelligence", text: "Post and comment summaries are generated on device from the selected visible text. They may be wrong, are shown separately from Reddit content, and do not use a hosted model. If Apple Intelligence is unavailable, Octonaut can show deterministic Key excerpts instead."),
        HelpSection(id: "privacy", title: "Privacy", text: "Octonaut has no analytics SDK, hosted AI service, developer-operated account service, or release-one push server. Reddit requests still disclose normal network information to Reddit and media hosts. Clear local content from Settings."),
